...
SuperInsights 是一个基于大模型Large Language Model的工具，用于从指定的网页中提取并总结关键信息。即使你不会写爬虫，也可以完全使用。它能够自动抓取网页内容，支持对接兼容 OpenAI 语言模型对其进行分析、总结以及翻译（如果需要）。
...
6. 运行示例脚本:
   ...
   python main.py
   ```
**7.系统启动后，你可以在shell中看到服务启动相关信息，但这时数据采集等相关任务并未启动，需要你手动执行，见下面使用指南。**

## 使用指南

程序运行后，可通过几个特定接口执行任务。**注意以下接口有执行顺序，即先update，然后generate，而后通过/api/summarydata取得数据。**
